...
export default async function UeberUnsPage() {
    ...
    return (
        <article className="min-h-screen bg-creme relative">
            {/*
            Background noise is handled globally in globals.css now
            The layout here represents a classic "Magazin-Stil" / Feuilleton with Apple UX typography
        */}

            {/* Hero Section (Sepia/Black&White) */}
            ...
            <div className="max-w-3xl mx-auto px-6 -mt-32 relative z-10 pb-32">
                ...
                {/* Magazin / Drop Cap Content Area */}
                {/* The first-letter CSS creates the drop cap (Initiale) effect */}
                <div className="
            ...
            ">
                    {/* 
                  Using dangerouslySetInnerHTML here assumes the WP data is sanitized. 
                  For demonstration, we split the mock content and ensure the first P gets the drop cap styling.
                */}
                    <div dangerouslySetInnerHTML={{ __html: sanitizeHtml(data.content) }} />
                </div>

                {/* Bottom Viennese Ornament (Simple classic divider) */}
