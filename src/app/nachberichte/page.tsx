...
import { getLatestNews } from "@/lib/mockData"; // using news stub for now as placeholder

export default async function NachberichteArchive() {
    const reports = await getLatestNews(); // in real life, getEventReports()
        ...
        <main className="min-h-screen bg-creme pb-32">
            <div className="pt-24 pb-16 px-6 max-w-5xl mx-auto text-center border-b border-gray-200/50">
                <h1 className="font-serif text-5xl md:text-6xl text-gray-900 mb-6 tracking-wide">
                    Veranstaltungsnachschauen
                </h1>
                <p className="font-sans text-lg text-gray-500 max-w-2xl mx-auto font-light leading-relaxed">
                    Rückblicke, Zusammenfassungen und Foto-Dokumentationen unserer vergangenen Salons.
                </p>
            </div>
            ...
            <div className="max-w-6xl mx-auto px-6 pt-16 mt-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16">
                    {reports.map((item) => (
                        <Link key={item.id} href={`/nachberichte/${item.id}`} className="group flex flex-col items-start">
                            {/* Image Wrapper */}
                            <div className="w-full aspect-[4/3] bg-gray-100 mb-6 overflow-hidden rounded-sm relative">
                                {item.imageUrl ? (
                                    <img
                                        src={item.imageUrl}
                                        alt={item.title}
                                        className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700 ease-in-out hover:grayscale-0 grayscale transition-all"
                                    />
                                ) : (
